...
ROC.CL.alfa<-function(yD, yH,pe,alfa=NULL){
	...
	################################################
	# ESTIMADORES CLASICOS de POSICION Y ESCALA
	################################################
	...
	######################################################
	# EMPIRICA CLASICA, proyectada en la direccion alfa
	#######################################################
...
###################################################
# LA GRILLA DONDE SE OBSERVAN LAS TRAYECTORIAS DEBE 
# SER LA MISMA
###################################################
###############################################################
...
ROC.CL.funcional.alfa<-function(grilla.tes, XD, XH,pe=seq(0,1,length=100),alfa=NULL, kn=10,varexp=1, norder=4, base="splines", base.verdad, pooled="SI"){
	...
	if(base=="PC"){
		...
		print(porcentaje)
    		...
    		normas   <- sqrt(colSums(pc.CL^2) * dt)  ####PARA TENER NORMA L2 1
	...
	if(base=="fourier"){
		...
		if(pepe==TRUE){
			...
			normas   <- sqrt(colSums(base.fourier.grilla^2) * dt)  ####PARA TENER NORMA L2 1

			base_proyec <-   base.fourier.grilla %*% diag(1 / normas)
		}
	}

        ## Estimated   coefficients en la base   (by row)
...
# MEDIANA L1  
###################################################################################
# Dado una coleccion de datos (matriz por fila), devuelta la mediana L1.
###################################################################################
